...
export class LZ77 {
    constructor(input, lazy) {
        this.table = {}; //chained-hash-table
        this.pos = 0;
        this.prevMatch = null; //previous longest match
        this.skipLength = 0; //skiplength
        this.lazy = 0;
        this.input = input;
    ...
    maxMatchTest(match1, match2, len) {
        for (var j = len; j > LZ77MinLength; j--) {
            if (this.input[match1 + j - 1] !== this.input[match2 + j - 1])
                return false;
        }
        return true;
    }
    ...
    searchLongestMatch(position, matchList) {
        ...
        matchMax = 0, inputLength = this.input.length;
        for (var i = 0; i < matchList.length; i++) {
            ...
            if (matchMax > LZ77MinLength) {
                var j = matchMax;
                if (!this.maxMatchTest(match, position, matchMax))
                    continue;
